...
//!   is `ResourceUrl::aux` (`src/space.rs:175-179`); `StorageSpace::resolve`'s
//!   auxiliary branch (`src/space.rs:259-279`) does not construct one
...
//!   request path shaped `/.aux/<segment>/<subject-path>` (`src/space.rs:263-267`),
//!   and a child name with no `/` can never supply the `segment/subject`
//!   split that shape needs. The deepest a slug-derived path can reach is the
//!   single reserved segment `/.aux` with no remainder, which `resolve`
//!   refuses as `SpaceError::Reserved` (`src/space.rs:260-265`, a 404 — see
//!   `src/http.rs`'s `classify`) — never a `Target::Aux`.
//! * **Auxiliary-of-an-auxiliary.** `AuxUrl`'s impl block (`src/space.rs:223-230`)
//!   offers only `subject()` and `kind()`; there is no method that returns
//!   another `AuxUrl`.
//! * **Twin traversals.** `ResourceUrl::ancestors` (`src/space.rs:203-211`) is
...
//!   call to it (`src/wac/guard.rs:135`), which is what stops the two from
...
//!   `DirectlyDeletable` is implemented only for `AuxUrl` (`src/space.rs:166`)
