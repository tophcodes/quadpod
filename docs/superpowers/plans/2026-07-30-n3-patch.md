...
pub async fn patch_dataset(
    ...
    let bindings = if patch.conditions().is_empty() {
        ...
        // `SELECT *`, not a projection list built from `patch.variables()`:
        // a condition may be entirely ground — `solid:where { <> ex:email "old" }`
        // is the ordinary conditional-insert idiom — and an empty projection
        // list is not legal SPARQL. The BGP's in-scope variables are exactly
        // `?v0..?v{n-1}`, so reading them back by name is unaffected.
        let solutions = store
...
### Task 6b: patching an auxiliary

`patch_impl` answers `501` for an `Aux` target, because `resource::patch_dataset` takes a
`ResourceUrl` and `AuxUrl` deliberately is not one. That constraint does not forbid writing an
ACL — the pod does it on every `PUT` — it forbids writing one **unguarded**:
`docs/constraints.md` records that `put_rdf(&foo.aux(Acl))` once compiled and skipped the
subject-existence guard, planting a policy document on a path that no longer existed, where
nearest-ACL-wins made it permanent.

So the patch path needs the same guard, not a different algorithm. `aux::conditional_put_update`
already has it in the right shape — a `FILTER EXISTS` **inside** the update, chosen because *"a
check and a write are two round-trips: a `DELETE` interleaving between them would let the write
land on a subject that no longer exists"*. The folded patch update already carries a `WHERE`, so
the guard goes into it and costs no extra round trip.

**Files:**
- Modify: `src/resource.rs` (extract the guarded form), `src/aux.rs` (the entry point), `src/http.rs` (route `Aux` to it), `docs/constraints.md` (one rule)
- Test: `src/aux.rs` `mod tests`, `src/http.rs` `mod tests`

**Interfaces:**
- Produces: `pub(crate) resource::patch_guarded(&dyn SparqlStore, &impl GraphName, &Patch, guard: &str) -> Result<PatchResult, ResourceError>`; `pub async fn aux::patch(&dyn SparqlStore, &AuxUrl, &Patch) -> Result<PatchResult, AuxError>`.
- `resource::patch_dataset` keeps its signature and becomes `patch_guarded(store, r, patch, "")`.

- [ ] **Step 1: Write the failing tests**

In `src/aux.rs`'s `mod tests`, modelled on the existing `aux::put` tests — read them first and
reuse their fixtures:

```rust
    #[tokio::test]
    async fn an_acl_can_be_patched_when_its_subject_exists() {
        // Create a subject resource, PUT an ACL on it granting Read, then
        // patch the ACL to add acl:Write. Assert PatchResult::Applied and that
        // the stored ACL graph now holds both modes.
    }

    // The reason this task exists. Without the guard a patch could write a
    // policy document onto a path that no longer exists, which is the defect
    // `docs/constraints.md` records for `put_rdf`.
    #[tokio::test]
    async fn patching_an_acl_whose_subject_is_gone_writes_nothing() {
        // Create a subject, PUT an ACL, delete the subject, then patch the ACL.
        // Assert Err(AuxError::SubjectMissing) AND that the ACL graph is
        // unchanged — asserted on the store, not on the error alone.
    }
```

In `src/http.rs`'s `mod tests`, using the existing `patch_n3` helper:

```rust
    #[tokio::test]
    async fn an_acl_url_accepts_a_patch() {
        // PUT /profile, PUT /.aux/profile.acl, then PATCH the ACL URL adding a
        // triple. Assert 204, then GET the ACL and assert the triple is there.
    }
    ...
    // holding Write on the subject but not Control must be refused.
    #[tokio::test]
    async fn patching_an_acl_needs_control_not_write() {
        // Model the ACL setup on the nearest existing WAC test.
    }

    #[tokio::test]
    async fn patching_an_acl_whose_subject_is_absent_is_404() {
        // PATCH /.aux/never-existed.acl → 404 with AUX_SUBJECT_MISSING_MESSAGE,
        // matching what PUT already answers.
    }
```

Write each one out fully against the real fixtures. A described test is not a test.

- [ ] **Step 2: Run tests to verify they fail**

Run: `nix develop -c cargo test --lib aux 2>&1 | tail -20` and
`nix develop -c cargo test --lib http::tests::an_acl_url 2>&1 | tail -20`
Expected: FAIL — `aux::patch` does not exist; the HTTP route answers `501`.

- [ ] **Step 3: Extract the guarded form**

In `src/resource.rs`, rename the body of `patch_dataset` into:
...
The body is today's `patch_dataset` with `r` replaced by `g` and the guard appended inside the
update's `WHERE` — `WHERE {{ {deletions} {guard} }}`. The counting `SELECT` and the labelling
`ASK` are unchanged: if the subject is gone the guard makes the update a no-op, which is what
must not be reported as success, and step 4 handles that.

`patch_dataset` becomes one line: `patch_guarded(store, r, patch, "").await`.

- [ ] **Step 4: Write `aux::patch`**

In `src/aux.rs`, beside `put`:
...
Build the same `FILTER EXISTS { GRAPH <subject_sys> { <subject_iri> <SYS_PRESENT> true } }`
fragment `conditional_put_update` builds — extract it into a small shared function in `aux.rs`
so the two cannot drift, rather than writing the string twice. Call
`resource::patch_guarded(store, aux, patch, &guard)`, then `exists(store, aux)`; if the
auxiliary is absent afterwards, return `AuxError::SubjectMissing`.

- [ ] **Step 5: Route it**

In `src/http.rs`, replace the `Target::Aux(_) => 501` branch with a call to `aux::patch`, mapping
`AuxError::SubjectMissing` to `404` with `AUX_SUBJECT_MISSING_MESSAGE` exactly as `put_impl`
does, and `PatchResult` to the same statuses the resource branch uses.

**Skip the `required_modes` check for an `Aux` target.** `authorize` already substituted
`Control` for it, and asking again with modes derived from the patch's parts would demand `Read`
or `Write` on top of `Control` — refusing an ACL patch from an agent WAC says may make it.
...
- [ ] **Step 7: Verify the guard bites**

Temporarily pass `""` as the guard from `aux::patch`. Run
`nix develop -c cargo test --lib aux::tests::patching_an_acl_whose_subject_is_gone`.
Expected: FAIL — the patch wrote to an orphaned ACL graph. Revert.

Without this the guard is a string nothing checks, and the defect it prevents is one this
project has already shipped once.

- [ ] **Step 8: Add the constraint**

Append to `docs/constraints.md`, in the "Boundaries that have no compiler behind them" section:
...
**Demonstrate it goes red:** add `let _ = crate::resource::patch_guarded;` to `src/http.rs`, run
`arch-check --only 'patches an auxiliary'`, confirm non-zero exit, remove it.

- [ ] **Step 9: Commit**

```bash
git add src/resource.rs src/aux.rs src/http.rs docs/constraints.md
git commit -m "feat: patch an auxiliary through its own guard

An ACL is an ordinary resource a client PUTs, so PATCH must reach it too. What
the type model forbids is writing one unguarded: put_rdf on an auxiliary once
compiled and skipped the subject-existence check, planting a policy document on
a path that no longer existed.

The guard is a FILTER EXISTS inside the update, the shape aux::put already
chose because a check and a write are two round-trips. The patch update already
has a WHERE, so the precondition costs nothing extra."
```
