...
`a_patch_that_may_bind_to_a_skolem_iri_parses_fine` passes already. That is intended: it is a regression guard for step 3, not a red-first test. Note it in the commit rather than pretending otherwise.
